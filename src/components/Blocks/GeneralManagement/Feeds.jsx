import { useEffect, useState } from "react";
import { useFunction } from "../../../context/FunctionContext";
import { useAuth } from "../../../context/authContext";
import { AiFillDelete, AiFillPlusCircle } from "react-icons/ai";
import { BigTextInput, Button, TextInput } from "../../Forms";
import { HiPencilAlt } from "react-icons/hi";
import { Modal, Alert } from "../../Containers";

export default function Feeds() {
  const [refresh, doRefresh] = useState(0);
  const [feeds, setFeeds] = useState([]);
  const [modalTitle, setModalTitle] = useState(null);
  const [selectedFeed, setFeed] = useState(null);
  const [alert, toggleAlert] = useState({
    type: "success",
    title: "",
    message: "",
    show: false,
  });
  const [newFeed, setNewFeed] = useState({
    name: 0,
    description: 0,
  });

  const { capitalize, toTitle } = useFunction();
  const { getFeeds, addFeeds, updateFeeds, deleteFeeds } = useAuth();

  const handleUpdate = async (e) => {
    e.preventDefault();
    const response = await updateFeeds(newFeed, selectedFeed.id);
    setModalTitle(null);
    if (response === 1) {
      toggleAlert({
        type: "success",
        title: "Feeds Update Complete",
        message: "Feeds information has been updated.",
        show: true,
      });
    } else {
      toggleAlert({
        type: "warning",
        title: "Update Error",
        message:
          "There has been an error on updating the feeds information. Please try again.",
        show: true,
      });
    }
    doRefresh((count) => (count = count + 1));
  };
  const handleRegistration = async (e) => {
    e.preventDefault();
    const response = await addFeeds(newFeed);
    setModalTitle(null);
    if (response === 1) {
      toggleAlert({
        type: "success",
        title: "Registration Complete",
        message: "New feeds has been registered.",
        show: true,
      });
    } else {
      toggleAlert({
        type: "warning",
        title: "Registration Error",
        message: "There has been an error on registration. Please try again.",
        show: true,
      });
    }
    doRefresh((count) => (count = count + 1));
  };
  const handleDelete = async () => {
    const response = await deleteFeeds(selectedFeed.id);
    setModalTitle(null);
    if (response == 1) {
      toggleAlert({
        type: "success",
        title: "Deletion Complete",
        message: `${capitalize(selectedFeed.name)} has been deleted.`,
        show: true,
      });
    } else {
      toggleAlert({
        type: "warning",
        title: "Deletion Error",
        message: "An error has occured. Please try again.",
        show: true,
      });
    }
    doRefresh((count) => (count = count + 1));
  };

  const handleClose = () => {
    setModalTitle(null);
    setFeed(null);
    toggleAlert({
      type: "success",
      title: "",
      message: "",
      show: false,
    });
    setNewFeed({ name: 0, description: 0 });
  };

  useEffect(() => {
    const setup = async () => {
      const response = await getFeeds();
      setFeeds(response);
    };
    setup();
   
  }, [refresh]);

  return (
    <>
      <div>
        <div className="flex flex-row items-center justify-between">
          <Button
            onClick={() => setModalTitle("add feeds")}
            value={
              <div className="flex items-center gap-1">
                <AiFillPlusCircle />
                <span>Add Feeds</span>
              </div>
            }
            className={
              "bg-main text-white p-1 px-2 rounded-full text-[.9rem] transition-all hover:bg-tertiary hover:text-main"
            }
          />
        </div>
        <div className="py-2">
          <table className="w-full rounded-md shadow-md overflow-hidden">
            <thead>
              <tr className="bg-main text-white">
                <th>Feed Name</th>
                <th>Description</th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {feeds.map((feed) => {
                return (
                  <tr key={feed.id} className="hover:bg-slate-200">
                    <td align="center" className="p-2 w-1/5">
                      {capitalize(feed.name)}
                    </td>
                    <td align="center" className="p-2 w-3/5">
                      {feed.description}
                    </td>
                    <td align="center" className="p-2 w-1/5">
                      <div className="flex items-center justify-center gap-1">
                        <Button
                          onClick={() => {
                            setFeed(feed);
                            setNewFeed({
                              name: feed.name,
                              description: feed.description,
                              update: true,
                            });
                            setModalTitle("edit feeds");
                          }}
                          className="bg-yellow p-1 rounded"
                          value={<HiPencilAlt className="text-white" />}
                        />
                        <Button
                          className="bg-red-light p-1 rounded"
                          onClick={() => {
                            setFeed(feed);
                            setModalTitle("confirmation");
                          }}
                          value={<AiFillDelete className="text-white" />}
                        />
                      </div>
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>
      </div>
      {modalTitle && (
        <Modal
          title={capitalize(modalTitle)}
          onClose={() => handleClose()}
          className="w-[95%] max-w-[450px]"
          content={
            ["add feeds", "edit feeds"].includes(modalTitle) ? (
              <>
                <form
                  autoComplete="off"
                  className="flex flex-col gap-2"
                  onSubmit={
                    Object.keys(newFeed).includes("update")
                      ? handleUpdate
                      : handleRegistration
                  }
                >
                  {Object.keys(newFeed)
                    .filter((key) => key !== "update")
                    .map((lbl, index) => {
                      return lbl === "name" ? (
                        <TextInput
                          key={index}
                          id={lbl}
                          value={
                            modalTitle === "edit feeds" ? newFeed[lbl] : null
                          }
                          withLabel={capitalize(toTitle(lbl))}
                          orientation="row"
                          classes="p-1 items-center justify-between"
                          labelClasses="whitespace-nowrap w-1/3 text-start"
                          inputClasses="bg-default w-2/3 rounded px-2"
                          onChange={(e) => {
                            setNewFeed((current) => ({
                              ...current,
                              [lbl]: e.target.value,
                            }));
                          }}
                        />
                      ) : (
                        <BigTextInput
                          key={index}
                          id={lbl}
                          value={
                            modalTitle === "edit feeds" ? newFeed[lbl] : null
                          }
                          withLabel={capitalize(toTitle(lbl))}
                          classes="p-1 items-start justify-between"
                          labelClasses="whitespace-nowrap w-1/3 text-start"
                          inputClasses="bg-default w-2/3 rounded px-2 min-w-[250px] min-h-[200px]"
                          onChange={(e) => {
                            setNewFeed((current) => ({
                              ...current,
                              [lbl]: e.target.value,
                            }));
                          }}
                        />
                      );
                    })}
                  <div className="flex items-center justify-end gap-2">
                    <Button
                      type="submit"
                      value={newFeed.update ? "Save Changes" : "Register"}
                      className="bg-tertiary p-1 px-2 rounded-md hover:bg-main hover:text-white transition-all"
                    />
                    <Button
                      value="Cancel"
                      onClick={() => handleClose()}
                      className="bg-gray-200 text-gray-700 p-1 px-2 rounded-md"
                    />
                  </div>
                </form>
              </>
            ) : (
              <>
                <p>
                  Are you sure to remove{" "}
                  <span className="font-semibold">
                    {capitalize(selectedFeed.name)}?
                  </span>
                </p>
                <div className="flex items-center justify-end gap-2">
                  <Button
                    value="Delete"
                    onClick={() => handleDelete()}
                    className="bg-tertiary p-1 px-2 rounded-md hover:bg-main hover:text-white transition-all"
                  />
                  <Button
                    value="Cancel"
                    onClick={() => handleClose()}
                    className="bg-gray-200 text-gray-700 p-1 px-2 rounded-md"
                  />
                </div>
              </>
            )
          }
        />
      )}
      {alert.show && (
        <Alert
          type={alert.type}
          message={alert.message}
          title={alert.title}
          onClose={() => handleClose()}
        />
      )}
    </>
  );
}
